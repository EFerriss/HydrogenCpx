...
Updated for 3mm instead of 1mm side cube
"""
...
data_90 = np.delete(data_90, idx_to_delete, 0)
...
# plot time versus rim loss data at 80 %  
for idx_D, rimloss in enumerate(data[1:-1]):
    ax.plot(time_hours, rimloss, '-', mew=1, linewidth=2, color=colors[idx_D],
            label=''.join(('logD=', '{:.1f}'.format(logD_list[idx_D]))))

for idx_D, rimloss in enumerate(data_90[1:-1]):
    ax.plot(time_hours, rimloss, '-', mew=1, linewidth=2, color=colors[idx_D],
...
ax.text(ax.get_xlim()[1]/2., 1450,  'hydrogen loss at the center', 
        ha='center', va='center')
#
##ax.legend(loc=4)
#labels = []
...
plt.savefig('C:\\Users\\Ferriss\\Documents\\CpxPaper\\Fig15A-3mm.tif')
